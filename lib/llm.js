...
async function attemptPrice(sol, profile, online) {
  ...
  const sys = [
    ...
    `- Reference only: ${sol.effort || "Moderate"} effort lands around $${band.low}-$${band.high}. You MUST adjust away from those exact numbers for this specific solution.`,
  ...
  try {
    const out = await callOpenRouter(
      [{ role: "system", content: sys }, { role: "user", content: user }],
      { maxTokens: 700, temperature: 0.55, online, timeoutMs: online ? 9000 : 12000 }
    );
...
export async function priceSolutions({ solutions, profile }) {
  ...
  // Process calls sequentially in groups of 2 to avoid throttling local sockets
