...
// STEP 3 — indicative pricing. ONE focused agent per solution, run in PARALLEL,
// each doing its own web search so the ten ranges come out distinct and grounded
// instead of one fallback band copied across the board.
async function attemptPrice(sol, profile, online) {
  ...
  const sys = [
    ...
    online
      ? "Search the web for real market comparables for THIS specific build, then return ONE indicative TOTAL price range in USD for a one-year engagement (total contract value, not monthly)."
      : "Reason ONE precise indicative TOTAL price range in USD for a one-year engagement (total contract value, not monthly) from THIS build's real components — integration count, data work, channels, automation depth.",
    "The range MUST be specific to THIS exact solution. Do NOT return a round generic band. Two different solutions must not land on the same numbers. Use realistic non-round figures (e.g. 21,500 - 37,000), not tidy multiples of 1,000 that look templated.",
    online
    ...
    `- Reference only: ${sol.effort || "Moderate"} effort usually lands somewhere around $${band.low}-$${band.high}. You MUST adjust away from those exact numbers for this specific solution; never echo the band verbatim.`,
    "Keep it sane for an SMB. Indicative budgeting figure, not a quote.",
    "Return ONLY JSON, no prose, no code fences:",
...
async function priceOneSolution(sol, profile) {
  const band = EFFORT_BANDS[sol.effort] || EFFORT_BANDS.Moderate;
  // 1) try market pricing with web search (short leash so a dead search can't hang the run)
  let r = await attemptPrice(sol, profile, true);
  // 2) if that failed, REASON a specific per-solution number with no web dependency (reliable + varied)
  if (!r.valid) r = await attemptPrice(sol, profile, false);
  // 3) only if both calls fail, fall back to the raw effort band
  if (!r.valid) return { low: band.low, high: band.high, basis: "scope", note: "Estimate from effort band.", sources: [] };
  return r;
}

export async function priceSolutions({ solutions, profile }) {
  const list = Array.isArray(solutions) ? solutions.slice(0, 10) : [];
  const results = await Promise.all(list.map(sol => priceOneSolution(sol, profile)));

  // dedup sources into a single numbered appendix
  const sources = [];
  const sourceKey = {};
  function refFor(src) {
...
export async function analyzeCompany({ profile }) {
  ...
  // Pricing runs as a separate parallel pass via /api/price, so the analysis
  // returns fast and never times out waiting on ten market searches.
  return parsed;
}
