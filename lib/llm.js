...
async function attemptPrice(sol, profile, online) {
  ...
  const sys = [
    ...
    "The range MUST be specific to THIS exact solution. Do NOT return a round generic band. Use realistic non-round figures (e.g. 21500 - 37000).",
    online
    ...
    "Keep it sane for an SMB. Return ONLY JSON, no prose, no code fences:",
  ...
  try {
    const out = await callOpenRouter(
      [{ role: "system", content: sys }, { role: "user", content: user }],
      { maxTokens: 700, temperature: 0.55, online, timeoutMs: online ? 8000 : 12000 }
    );
...
async function priceOneSolution(sol, profile) {
  ...
  // Try live web search first, limit time aggressively
  let r = await attemptPrice(sol, profile, true);
  // Fallback to offline structural reasoning
  if (!r.valid) r = await attemptPrice(sol, profile, false);
  // Fallback to hardcoded safe configuration limits
...
export async function priceSolutions({ solutions, profile }) {
  ...
  // Fire requests concurrently but catch timeouts cleanly to prevent blocking the UI
